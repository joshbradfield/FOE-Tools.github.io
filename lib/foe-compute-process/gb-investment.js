...
function levelInvestment(
  ...
  for (let i = 0; i < gb[currentLevel - 1].reward.length && !levelCostReached; i++) {
    ...
    if (
      ...
      (securePlaceValueCurrentInvestment.fp > 0 && securePlaceYourParticipation >= securePlaceValueCurrentInvestment.fp)
    ) {
    ...
    if (remainingDefaultParticipation.length) {
      securePlaceOtherParticipation = !remainingDefaultParticipation[0].isPotentialSniper
        ? remainingDefaultParticipation[0].value
        : remainingDefaultParticipation.length > 1
        ? remainingDefaultParticipation[1].value
        : 0;
    }
...
export default {
  ...
  ComputeLevelInvestment(
    ...
    if (!(defaultParticipation instanceof Array)) {
      throw new Errors.InvalidTypeError({
        expected: "Array",
