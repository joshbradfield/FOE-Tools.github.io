...
function levelInvestment(
  ...
  const totalCurrentInvestment = ownerPreparation + defaultParticipation.reduce((i, j) => i + j, 0);
  ...
  for (let i = 0; i < gb[currentLevel - 1].reward.length && !levelCostReached; i++) {
    ...
    if (
      ...
      normalizedDefaultParticipation[defaultParticipationIndex] >= securePlaceValueCurrentInvestment.fp
    ) {
      investment.participation = normalizedDefaultParticipation[defaultParticipationIndex];
      investment.isInvestorParticipation = true;
      investment.roi = investment.expectedParticipation - normalizedDefaultParticipation[defaultParticipationIndex];
    ...
    const otherParticipation = investment.isInvestorParticipation
      ? investment.participation
      : defaultParticipationIndex < normalizedDefaultParticipation.length
  ...
  for (let i = result.investment.length; i < gb[currentLevel - 1].reward.length; i++) {
    ...
    if (defaultParticipationIndex < normalizedDefaultParticipation.length) {
      ...
      result.investment[result.investment.length - 1].roi =
        result.investment[result.investment.length - 1].expectedParticipation -
        normalizedDefaultParticipation[defaultParticipationIndex];
