export default {
   ...
   * @param cumulativeQuest {number} Number of cumulative quests. Used when we gain more coins and supplies than you use in quests.
   ...
   * coinSupplyReturn: Array, infinityGenerator: boolean, defaultCumulativeQuest: number}} All awards obtained statistically.
   */
  compute(age, yourCfBoost, coins, supplies, goods, fpBy24h, secondRq, suppliesGathered, otherRq, cumulativeQuest = 0) {
    let cf_boost = yourCfBoost / 100;
    let coins_relation = Math.floor(coins / age.cost.coins);
    let supplies_relation = Math.floor(supplies / age.cost.supplies);
    ...
    do {
      ...
      if (coin_return > 0 || supply_return > 0) {
        if (
          !result.infinityGenerator &&
          result.coinSupplyReturn.length > 0 &&
          result.coinSupplyReturn[result.coinSupplyReturn.length - 1].coin <= coin_return
        ) {
      ...
      (!result.infinityGenerator && (coin_return > age.cost.coins || supply_return > age.cost.supply)) ||
      (result.infinityGenerator && cumulativeQuest > result.coinSupplyReturn.length)
    );

    if (result.infinityGenerator && cumulativeQuest === 0) {
      result.defaultCumulativeQuest = result.coinSupplyReturn.length + 1;
    } else if (!result.infinityGenerator) {
